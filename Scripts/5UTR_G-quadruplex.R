###the sequence mapping and calculation of G-quadruplex in 5'UTR
...
######################### 5'UTR annotation information containing genome coordinates:
...
gff_5utr = mygff[mygff$type == 'five_prime_UTR',]
gff_5utr = gff_5utr[, c(1,3,4,5,7,9)]




##the sequence mapping and calculation of G-quadruplex in 5'UTR:
##(based on the genome coordinates of G-quadruplex and genome coordinates of 5'UTR)
gff_5utr$GQnumber = 0

gff_5utr$TemplateStrand = 0
gff_5utr$CodingStrand = 0
gff_5utr$SCORE_TemplateStrand = NA
gff_5utr$SCORE_CodingStrand = NA


for (i in 1:nrow(gff_5utr)){

  for (j in 1: nrow(G4Hunter)){
    
    if (gff_5utr$seq_id[i] == G4Hunter$ID_Chr[j]){
      
      if (min(gff_5utr$end[i], G4Hunter$end[j]) >= max(gff_5utr$start[i], G4Hunter$start[j])){
        
        if (gff_5utr$strand[i] == '+' & G4Hunter$SCORE[j] > 0){
          gff_5utr$TemplateStrand[i] = gff_5utr$TemplateStrand[i] + 1
          gff_5utr$GQnumber[i] = gff_5utr$GQnumber[i] + 1
          gff_5utr$SCORE_TemplateStrand[i] = paste(gff_5utr$SCORE_TemplateStrand[i], G4Hunter$SCORE[j], sep = ", ")  
          
          
        }  
        if (gff_5utr$strand[i] == '+' & G4Hunter$SCORE[j] < 0){
          gff_5utr$CodingStrand[i] = gff_5utr$CodingStrand[i] + 1
          gff_5utr$GQnumber[i] = gff_5utr$GQnumber[i] + 1
          gff_5utr$SCORE_CodingStrand[i] = paste(gff_5utr$SCORE_CodingStrand[i], G4Hunter$SCORE[j], sep = ", ") 
        }
        if (gff_5utr$strand[i] == '-'  & G4Hunter$SCORE[j] > 0){
          gff_5utr$CodingStrand[i] = gff_5utr$CodingStrand[i] + 1
          gff_5utr$GQnumber[i] = gff_5utr$GQnumber[i] + 1
        ...
        if(G4Hunter$start[j] > gff_5utr$end[i]){
          break
        }
...
write.table (gff_5utr, file ="5UTR_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)

###the sequence mapping and calculation of G-quadruplex in CDS
...
######################### CDS annotation information containing genome coordinates:
...
gff_CDS = mygff[mygff$type == 'CDS',]
gff_CDS = gff_CDS[, c(1,3,4,5,7,9)]




##the sequence mapping and calculation of G-quadruplex in CDS:
##(based on the genome coordinates of G-quadruplex and genome coordinates of CDS)
gff_CDS$GQnumber = 0

gff_CDS$TemplateStrand = 0
gff_CDS$CodingStrand  = 0
gff_CDS$SCORE_TemplateStrand = NA
gff_CDS$SCORE_CodingStrand  = NA


for (i in 1:nrow(gff_CDS)){
  
  for (j in 1: nrow(G4Hunter)){
    
    if (gff_CDS$seq_id[i] == G4Hunter$ID_Chr[j]){
      
      
      if (min(gff_CDS$end[i], G4Hunter$end[j]) >= max(gff_CDS$start[i], G4Hunter$start[j])){
        
        if (gff_CDS$strand[i] == '+' & G4Hunter$SCORE[j] > 0){
          gff_CDS$TemplateStrand[i] = gff_CDS$TemplateStrand[i] + 1
          gff_CDS$GQnumber[i] = gff_CDS$GQnumber[i] + 1
          gff_CDS$SCORE_TemplateStrand[i] = paste(gff_CDS$SCORE_TemplateStrand[i], G4Hunter$SCORE[j], sep = ", ")  
          
          
        }  
        if (gff_CDS$strand[i] == '+' & G4Hunter$SCORE[j] < 0){
          gff_CDS$CodingStrand [i] = gff_CDS$CodingStrand [i] + 1
          gff_CDS$GQnumber[i] = gff_CDS$GQnumber[i] + 1
          gff_CDS$SCORE_CodingStrand [i] = paste(gff_CDS$SCORE_CodingStrand [i], G4Hunter$SCORE[j], sep = ", ") 
        }
        if (gff_CDS$strand[i] == '-'  & G4Hunter$SCORE[j] > 0){
          gff_CDS$CodingStrand [i] = gff_CDS$CodingStrand [i] + 1
          gff_CDS$GQnumber[i] = gff_CDS$GQnumber[i] + 1
        ...
        if(G4Hunter$start[j] > gff_CDS$end[i]){
          break
        }
...
write.table (gff_CDS, file ="CDS_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)

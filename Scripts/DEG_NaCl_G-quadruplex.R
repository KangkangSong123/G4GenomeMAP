###the sequence mapping and calculation of G-quadruplex in all feature regions under NaCl stress condition 
...
diff_N = read.csv("diff_NaCl.csv", encoding="UTF-8")
diff_N$gene = substr(diff_N$gene, 1, 21)
diff_N = diff_N[diff_N$group != 'NOT_CHANGE',]
...
n = 1
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_promoter)){
    if (diff_N$gene[i] == gff_promoter$gene_id[j]){
      gff_promoter2[n, ] = gff_promoter[j, ]
      gff_promoter2$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}
...
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_promoter_1500)){
    if (diff_N$gene[i] == gff_promoter_1500$gene_id[j]){
      gff_promoter_15002[n, ] = gff_promoter_1500[j, ]
      gff_promoter_15002$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}
...
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_promoter_1000)){
    if (diff_N$gene[i] == gff_promoter_1000$gene_id[j]){
      gff_promoter_10002[n, ] = gff_promoter_1000[j, ]
      gff_promoter_10002$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}
...
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_promoter_500)){
    if (diff_N$gene[i] == gff_promoter_500$gene_id[j]){
      gff_promoter_5002[n, ] = gff_promoter_500[j, ]
      gff_promoter_5002$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}
...
write.table (DEG_GQ_promoter_500, file ="DEG_promoter500_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)
...
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_TSS_updown500)){
    if (diff_N$gene[i] == gff_TSS_updown500$gene_id[j]){
      gff_TSS_updown5002[n, ] = gff_TSS_updown500[j, ]
      gff_TSS_updown5002$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}
...
write.table (DEG_GQ_TSS_updown500, file ="DEG_TSS500_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)
...
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_5UTR)){
    if (diff_N$gene[i] == gff_5UTR$gene_id[j]){
      gff_5UTR2[n, ] = gff_5UTR[j, ]
      gff_5UTR2$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}
...
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_3UTR)){
    if (diff_N$gene[i] == gff_3UTR$gene_id[j]){
      gff_3UTR2[n, ] = gff_3UTR[j, ]
      gff_3UTR2$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}
...
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_exon)){
    if (diff_N$gene[i] == gff_exon$gene_id[j]){
      gff_exon2[n, ] = gff_exon[j, ]
      gff_exon2$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}
...
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_CDS)){
    if (diff_N$gene[i] == gff_CDS$gene_id[j]){
      gff_CDS2[n, ] = gff_CDS[j, ]
      gff_CDS2$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}
...
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_intron)){
    if (diff_N$gene[i] == gff_intron$gene_id[j]){
      gff_intron2[n, ] = gff_intron[j, ]
      gff_intron2$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}
...
for (i in 1:nrow(diff_N)){
  for (j in 1:nrow(gff_gene)){
    if (diff_N$gene[i] == gff_gene$gene_id[j]){
      gff_gene2[n, ] = gff_gene[j, ]
      gff_gene2$group[n] = diff_N$group[i]
      n = n + 1
    }
  }
}

###the sequence mapping and calculation of G-quadruplex in TSS500
...
######################### TSS500 annotation information containing genome coordinates:
...
for (i in 1:nrow(gff_promoter)){
  if (gff_promoter$strand[i] == '+'){
    gff_promoter$start_promoter[i] = gff_promoter$start[i] - 250
    gff_promoter$end_promoter[i] = gff_promoter$start[i] + 249
  }
  if (gff_promoter$strand[i] == '-'){
    gff_promoter$start_promoter[i] = gff_promoter$end[i] -249
    gff_promoter$end_promoter[i] = gff_promoter$end[i] + 250
...
##the sequence mapping and calculation of G-quadruplex in TSS500:
##(based on the genome coordinates of G-quadruplex and genome coordinates of TSS500)
...
write.table (gff_promoter, file ="TSS500_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)

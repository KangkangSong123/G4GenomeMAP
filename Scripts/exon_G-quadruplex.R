###the sequence mapping and calculation of G-quadruplex in exon
...
######################### exon annotation information containing genome coordinates:
...
gff_exon = mygff[mygff$type == 'exon',]
gff_exon = gff_exon[, c(1,3,4,5,7,9)]
...
##the sequence mapping and calculation of G-quadruplex in exon:
##(based on the genome coordinates of G-quadruplex and genome coordinates of exon)
gff_exon$GQnumber = 0

gff_exon$TemplateStrand = 0
gff_exon$CodingStrand = 0 
gff_exon$SCORE_TemplateStrand = NA
gff_exon$SCORE_CodingStrand = NA


for (i in 1:nrow(gff_exon)){
  
  for (j in 1: nrow(G4Hunter)){
    
    if (gff_exon$seq_id[i] == G4Hunter$ID_Chr[j]){
      
      
      if (min(gff_exon$end[i], G4Hunter$end[j]) >= max(gff_exon$start[i], G4Hunter$start[j])){
        
        if (gff_exon$strand[i] == '+' & G4Hunter$SCORE[j] > 0){
          gff_exon$TemplateStrand[i] = gff_exon$TemplateStrand[i] + 1
          gff_exon$GQnumber[i] = gff_exon$GQnumber[i] + 1
          gff_exon$SCORE_TemplateStrand[i] = paste(gff_exon$SCORE_TemplateStrand[i], G4Hunter$SCORE[j], sep = ", ")  
          
          
        }  
        if (gff_exon$strand[i] == '+' & G4Hunter$SCORE[j] < 0){
          gff_exon$CodingStrand[i] = gff_exon$CodingStrand[i] + 1
          gff_exon$GQnumber[i] = gff_exon$GQnumber[i] + 1
          gff_exon$SCORE_CodingStrand[i] = paste(gff_exon$SCORE_CodingStrand[i], G4Hunter$SCORE[j], sep = ", ") 
        }
        if (gff_exon$strand[i] == '-'  & G4Hunter$SCORE[j] > 0){
          gff_exon$CodingStrand[i] = gff_exon$CodingStrand[i] + 1
          gff_exon$GQnumber[i] = gff_exon$GQnumber[i] + 1
        ...
        if(G4Hunter$start[j] > gff_exon$end[i]){
          break
        }
...
write.table (gff_exon, file ="exon_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)

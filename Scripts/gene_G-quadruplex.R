###the sequence mapping and calculation of G-quadruplex in gene
...
######################### gene annotation information containing genome coordinates:
...
##the sequence mapping and calculation of G-quadruplex in gene:
##(based on the genome coordinates of G-quadruplex and genome coordinates of gene)
gff_gene$GQnumber = 0

gff_gene$TemplateStrand = 0
gff_gene$CodingStrand = 0
gff_gene$SCORE_TemplateStrand = NA
gff_gene$SCORE_CodingStrand = NA

for (i in 1:nrow(gff_gene)){
  
  for (j in 1: nrow(G4Hunter)){
    
    if (gff_gene$seq_id[i] == G4Hunter$ID_Chr[j]){
      
      
      if (min(gff_gene$end[i], G4Hunter$end[j]) >= max(gff_gene$start[i], G4Hunter$start[j])){
        
        if (gff_gene$strand[i] == '+' & G4Hunter$SCORE[j] > 0){
          gff_gene$TemplateStrand[i] = gff_gene$TemplateStrand[i] + 1
          gff_gene$GQnumber[i] = gff_gene$GQnumber[i] + 1
          gff_gene$SCORE_TemplateStrand[i] = paste(gff_gene$SCORE_TemplateStrand[i], G4Hunter$SCORE[j], sep = ", ")
          
          
        }  
        if (gff_gene$strand[i] == '+' & G4Hunter$SCORE[j] < 0){
          gff_gene$CodingStrand[i] = gff_gene$CodingStrand[i] + 1
          gff_gene$GQnumber[i] = gff_gene$GQnumber[i] + 1
          gff_gene$SCORE_CodingStrand[i] = paste(gff_gene$SCORE_CodingStrand[i], G4Hunter$SCORE[j], sep = ", ") 
        }
        if (gff_gene$strand[i] == '-'  & G4Hunter$SCORE[j] > 0){
          gff_gene$CodingStrand[i] = gff_gene$CodingStrand[i] + 1
          gff_gene$GQnumber[i] = gff_gene$GQnumber[i] + 1
...
write.table (gff_gene, file ="gene_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)

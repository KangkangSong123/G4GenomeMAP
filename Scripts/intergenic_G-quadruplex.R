###the sequence mapping and calculation of G-quadruplex in intergenic
...
######################### intergenic_chain1 annotation information containing genome coordinates:
...
mygff = mygff[which(mygff$type =='gene'), ] 
mygff= mygff[which(mygff$strand =='+'), ]

gff_intergenic = mygff[order(mygff$seq_id, mygff$start), ]


gff_intergenic$type = 'intergenic'
gff_intergenic = gff_intergenic[, c(1,3,4,5,7,9)]


colnames(gff_intergenic)[3] <- 'gene_start'
colnames(gff_intergenic)[4] <- 'gene_end'
gff_intergenic$start = NA
gff_intergenic$end = NA

for (i in 1:nrow(gff_intergenic)){
  if(i < nrow(gff_intergenic)){
    if( gff_intergenic$seq_id[i] == gff_intergenic$seq_id[i+1]){
      gff_intergenic$start[i] = gff_intergenic$gene_end[i] + 1
      gff_intergenic$end[i] = gff_intergenic$gene_start[i+1] - 1 
    }
...
for (i in 1:nrow(gff_intergenic)){
  if(i < nrow(gff_intergenic)){
    if(gff_intergenic$seq_id[i] != gff_intergenic$seq_id[i+1]) {
      gff_intergenic = gff_intergenic[-i, ]
    }
  }
  if(i == nrow(gff_intergenic)){
    gff_intergenic = gff_intergenic[-i, ]
...
gff_intergenic = gff_intergenic[, c(1,2,7,8,5,6)]





##the sequence mapping and calculation of G-quadruplex in intergenic_chain1:
##(based on the genome coordinates of G-quadruplex and genome coordinates of intergenic_chain1)
gff_intergenic$GQnumber = 0

gff_intergenic$TemplateStrand = 0
gff_intergenic$CodingStrand = 0 
gff_intergenic$SCORE_TemplateStrand = NA
gff_intergenic$SCORE_CodingStrand = NA

for (i in 1:nrow(gff_intergenic)){
  
  for (j in 1: nrow(G4Hunter)){
    
    if (gff_intergenic$seq_id[i] == G4Hunter$ID_Chr[j]){

      
      if (min(gff_intergenic$end[i], G4Hunter$end[j]) >= max(gff_intergenic$start[i], G4Hunter$start[j])){
        
        if (gff_intergenic$strand[i] == '+' & G4Hunter$SCORE[j] > 0){
          gff_intergenic$TemplateStrand[i] = gff_intergenic$TemplateStrand[i] + 1
          gff_intergenic$GQnumber[i] = gff_intergenic$GQnumber[i] + 1
          gff_intergenic$SCORE_TemplateStrand[i] = paste(gff_intergenic$SCORE_TemplateStrand[i], G4Hunter$SCORE[j], sep = ", ")  
          
          
        }  
        if (gff_intergenic$strand[i] == '+' & G4Hunter$SCORE[j] < 0){
          gff_intergenic$CodingStrand[i] = gff_intergenic$CodingStrand[i] + 1
          gff_intergenic$GQnumber[i] = gff_intergenic$GQnumber[i] + 1
          gff_intergenic$SCORE_CodingStrand[i] = paste(gff_intergenic$SCORE_CodingStrand[i], G4Hunter$SCORE[j], sep = ", ") 
        }
        if (gff_intergenic$strand[i] == '-'  & G4Hunter$SCORE[j] > 0){
          gff_intergenic$CodingStrand[i] = gff_intergenic$CodingStrand[i] + 1
          gff_intergenic$GQnumber[i] = gff_intergenic$GQnumber[i] + 1
...
gff_intergenic_chain1 = gff_intergenic
rm(i,j, gff_intergenic)



######################### intergenic_chain2 annotation information containing genome coordinates:
mygff = read.table('gene_models_Chr_2017.gff',sep = '\t',header = F,check.names = F)
colnames(mygff) = c('seq_id','source','type','start','end','score','strand','phase','attributes')
...
row.names(gff_intergenic) = c(1:nrow(gff_intergenic))
gff_intergenic = gff_intergenic[-4163, ]
##The genes in lines 4162 and 4163 of the dataframe are overlapping genes.
row.names(gff_intergenic) = c(1:nrow(gff_intergenic))
...
##the sequence mapping and calculation of G-quadruplex in intergenic_chain2:
##(based on the genome coordinates of G-quadruplex and genome coordinates of intergenic_chain2)
gff_intergenic$GQnumber = 0
...
gff_intergenic_chain2 = gff_intergenic
rm(i,j, gff_intergenic)

gff_intergenic = rbind(gff_intergenic_chain1, gff_intergenic_chain2)

write.table (gff_intergenic, file ="intergenic_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)

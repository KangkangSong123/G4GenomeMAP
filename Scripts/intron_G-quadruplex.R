###the sequence mapping and calculation of G-quadruplex in intron
...
######################### intron annotation information containing genome coordinates:
...
for (i in 1:nrow(mygff)) {
  ...
  if((i+1) < nrow(mygff)){
    ...
    if(mygff$type[i]=='exon' & mygff$type[i+1]=='exon'){
      if(mygff$end[i] < mygff$end[i+1]){
        df_temp = mygff[i,]
        df_temp$type = 'intron'
        df_temp$start = mygff$end[i] + 1
        df_temp$end = mygff$start[i+1] - 1 
        
        gff_intron[m,] = df_temp
        m = m + 1
      }
      else{
        df_temp = mygff[i,] 
        df_temp$type = 'intron'
        df_temp$start = mygff$end[i+1] + 1
        df_temp$end = mygff$start[i] - 1
...
gff_intron = gff_intron[, c(1,3,4,5,7,9)]
...
##the sequence mapping and calculation of G-quadruplex in intron:
##(based on the genome coordinates of G-quadruplex and genome coordinates of intron)
gff_intron$GQnumber = 0

gff_intron$TemplateStrand = 0
gff_intron$CodingStrand = 0
gff_intron$SCORE_TemplateStrand = NA
gff_intron$SCORE_CodingStrand = NA


for (i in 1:nrow(gff_intron)){
  
  for (j in 1: nrow(G4Hunter)){
    
    if (gff_intron$seq_id[i] == G4Hunter$ID_Chr[j]){
     
      
      if (min(gff_intron$end[i], G4Hunter$end[j]) >= max(gff_intron$start[i], G4Hunter$start[j])){
        
        if (gff_intron$strand[i] == '+' & G4Hunter$SCORE[j] > 0){
          gff_intron$TemplateStrand[i] = gff_intron$TemplateStrand[i] + 1
          gff_intron$GQnumber[i] = gff_intron$GQnumber[i] + 1
          gff_intron$SCORE_TemplateStrand[i] = paste(gff_intron$SCORE_TemplateStrand[i], G4Hunter$SCORE[j], sep = ", ")  
          
          
        }  
        if (gff_intron$strand[i] == '+' & G4Hunter$SCORE[j] < 0){
          gff_intron$CodingStrand[i] = gff_intron$CodingStrand[i] + 1
          gff_intron$GQnumber[i] = gff_intron$GQnumber[i] + 1
          gff_intron$SCORE_CodingStrand[i] = paste(gff_intron$SCORE_CodingStrand[i], G4Hunter$SCORE[j], sep = ", ") 
        }
        if (gff_intron$strand[i] == '-'  & G4Hunter$SCORE[j] > 0){
          gff_intron$CodingStrand[i] = gff_intron$CodingStrand[i] + 1
          gff_intron$GQnumber[i] = gff_intron$GQnumber[i] + 1
        ...
        if(G4Hunter$start[j] > gff_intron$end[i]){
          break
        }
...
write.table (gff_intron, file ="intron_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)

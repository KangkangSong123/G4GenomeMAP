###the sequence mapping and calculation of G-quadruplex in promoter_1000
...
######################### promoter_1000 annotation information containing genome coordinates:
...
for (i in 1:nrow(gff_promoter)){
  if (gff_promoter$strand[i] == '+'){
    gff_promoter$start_promoter[i] = gff_promoter$start[i] - 1000
    gff_promoter$end_promoter[i] = gff_promoter$start[i] - 1
  }
  if (gff_promoter$strand[i] == '-'){
    gff_promoter$start_promoter[i] = gff_promoter$end[i] + 1
    gff_promoter$end_promoter[i] = gff_promoter$end[i] + 1000
...
##the sequence mapping and calculation of G-quadruplex in promoter_1000:
##(based on the genome coordinates of G-quadruplex and genome coordinates of promoter_1000)
...
write.table (gff_promoter, file ="promoter1000_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)

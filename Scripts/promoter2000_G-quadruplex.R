###the sequence mapping and calculation of G-quadruplex in promoter_2000
...
######################### promoter_2000 annotation information containing genome coordinates:
...
for (i in 1:nrow(gff_promoter)){
  if (gff_promoter$strand[i] == '+'){
    gff_promoter$start_promoter[i] = gff_promoter$start[i] - 2000
    gff_promoter$end_promoter[i] = gff_promoter$start[i] - 1
  }
  if (gff_promoter$strand[i] == '-'){
    gff_promoter$start_promoter[i] = gff_promoter$end[i] + 1
    gff_promoter$end_promoter[i] = gff_promoter$end[i] + 2000
...
##the sequence mapping and calculation of G-quadruplex in promoter_2000:
##(based on the genome coordinates of G-quadruplex and genome coordinates of promoter_2000)
...
write.table (gff_promoter, file ="promoter2000_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)

###the sequence mapping and calculation of G-quadruplex in promoter_500
...
######################### promoter_500 annotation information containing genome coordinates:
...
for (i in 1:nrow(gff_promoter)){
  if (gff_promoter$strand[i] == '+'){
    gff_promoter$start_promoter[i] = gff_promoter$start[i] - 500
    gff_promoter$end_promoter[i] = gff_promoter$start[i] - 1
  }
  if (gff_promoter$strand[i] == '-'){
    gff_promoter$start_promoter[i] = gff_promoter$end[i] + 1
    gff_promoter$end_promoter[i] = gff_promoter$end[i] + 500
...
##the sequence mapping and calculation of G-quadruplex in promoter_500:
##(based on the genome coordinates of G-quadruplex and genome coordinates of promoter_500)
...
write.table (gff_promoter, file ="promoter500_G-quadruplex", sep ="\t", row.names = FALSE, col.names =TRUE, quote = FALSE)
